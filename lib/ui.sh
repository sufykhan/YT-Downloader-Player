...
play_cmd() {
  ...
  if [ -n "$query" ]; then
    ...
    # If exactly one match, play it immediately
    if [ "${#matches[@]}" -eq 1 ]; then
      ...
      if [ -x "$HOME/scripts/music_manager/lib/play_helper.sh" ]; then
        ...
        _play_with_mpv "${matches[0]}"
      fi
      return 0
    fi

    # multiple matches -> fall back to interactive filter
    if _have_gum; then
    ...
    # no query -> interactive chooser over full index
  ...
  # convert selection into array safely and hand off to helper
